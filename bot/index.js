...
const CONFIG = {
  ...
  // Reaction Role System Configuration
  REACTION_ROLE_CHANNEL_ID: "", // Channel where reaction role panels will be posted (optional)
};
...
const reactionRolePanels = new Map(); // Map of message IDs to panel configurations
const reactionRoles = new Map(); // Map of panel IDs to their role configurations
...
client.on("messageCreate", async (message) => {
  ...
  if (
    ...
    // Validate emoji (accepts most Unicode emojis)
    const emojiRegex = /\p{Emoji}/u;
  ...
  if (message.content.startsWith("!create-rr-panel") && message.member.permissions.has(PermissionFlagsBits.Administrator)) {
    const args = message.content.split(" ").slice(1);
    const title = args.join(" ") || "Select Your Roles";

    const embed = new EmbedBuilder()
      .setColor("#5865F2")
      .setTitle(title)
      .setDescription("Click the buttons below to get or remove roles!\n\n*No roles configured yet. Use `!add-rr` to add roles.*")
      .setFooter({ text: `${message.guild.name} Role Selection` })
      .setTimestamp();

    const sentMessage = await message.channel.send({ embeds: [embed] });
    
    // Store panel info
    reactionRolePanels.set(sentMessage.id, {
      channelId: message.channel.id,
      title: title,
      roles: []
    });

    message.reply(`✅ Reaction role panel created! Message ID: \`${sentMessage.id}\`\nUse \`!add-rr ${sentMessage.id} @role emoji description\` to add roles.`);
  }

  // Add Role to Panel
  if (message.content.startsWith("!add-rr") && message.member.permissions.has(PermissionFlagsBits.ManageRoles)) {
    const role = message.mentions.roles.first();
    
    if (!role) {
      return message.reply("❌ Usage: `!add-rr <message_id> @role <emoji> [description]`\nExample: `!add-rr 123456789 @Member 🎮 Get this role for gaming updates`\n\n**Note:** You must actually mention the role (type @ and select it), not paste the role ID!");
    }

    // Remove the role mention from the content to parse other args
    const contentWithoutRole = message.content.replace(/<@&\d+>/, '').trim();
    const args = contentWithoutRole.split(/\s+/);
    ...
    if (!emoji || emoji.trim() === "") {
      return message.reply("❌ Please provide an emoji!\n**Usage:** `!add-rr <message_id> @role <emoji> [description]`\n**Example:** `!add-rr 123456789 @Member 🎮 Get this role for gaming updates`");
    }

    // Validate emoji (accepts most Unicode emojis)
    const emojiRegex = /\p{Emoji}/u;
    if (!emojiRegex.test(emoji)) {
      return message.reply(`❌ Invalid emoji! Please use a standard Unicode emoji.\nYou provided: "${emoji}"`);
    }

    const panel = reactionRolePanels.get(messageId);
    if (!panel) {
      return message.reply("❌ Panel not found! Make sure the message ID is correct.");
    }

    // Check if role already exists in panel
    if (panel.roles.some(r => r.roleId === role.id)) {
      return message.reply("❌ This role is already in the panel!");
    }

    // Check if we have too many roles (Discord button limit is 25 per message)
    if (panel.roles.length >= 25) {
      return message.reply("❌ This panel has reached the maximum of 25 roles!");
    }
    ...
    try {
      ...
      let roleList = "Click the buttons below to get or remove roles!\n\n";
      panel.roles.forEach(r => {
        const roleObj = message.guild.roles.cache.get(r.roleId);
        roleList += `${r.emoji} **${roleObj.name}** - ${r.description}\n`;
      });
      ...
      panel.roles.forEach((r, index) => {
        ...
        const roleObj = message.guild.roles.cache.get(r.roleId);
        const button = new ButtonBuilder()
          .setCustomId(`rr_${messageId}_${r.roleId}`)
      ...
      message.reply(`✅ Added role **${role.name}** to the panel!`);
    } catch (error) {
      console.error("Error updating panel:", error);
      message.reply("❌ Failed to update the panel!");
    }
  }

  // Remove Role from Panel
  if (message.content.startsWith("!remove-rr") && message.member.permissions.has(PermissionFlagsBits.ManageRoles)) {
    const args = message.content.split(" ");
    
    if (args.length < 3) {
      return message.reply("❌ Usage: `!remove-rr <message_id> @role`");
    }
    ...
    if (roleIndex === -1) {
      return message.reply("❌ This role is not in the panel!");
    }
    ...
    try {
      ...
      if (panel.roles.length === 0) {
        ...
        await panelMessage.edit({ embeds: [updatedEmbed], components: [] });
      } else {
        // Rebuild the panel
        let roleList = "Click the buttons below to get or remove roles!\n\n";
        panel.roles.forEach(r => {
      ...
      message.reply(`✅ Removed role **${role.name}** from the panel!`);
    } catch (error) {
      console.error("Error updating panel:", error);
      message.reply("❌ Failed to update the panel!");
    }
  ...
  if (message.content === "!list-rr-panels" && message.member.permissions.has(PermissionFlagsBits.Administrator)) {
    if (reactionRolePanels.size === 0) {
      return message.reply("❌ No reaction role panels found!");
    }

    let panelList = "**Reaction Role Panels:**\n\n";
    
    for (const [messageId, panel] of reactionRolePanels) {
      panelList += `📋 **${panel.title}**\nMessage ID: \`${messageId}\`\nChannel: <#${panel.channelId}>\nRoles: ${panel.roles.length}\n\n`;
    }

    message.reply(panelList);
  }

  // Delete Reaction Role Panel
  if (message.content.startsWith("!delete-rr-panel") && message.member.permissions.has(PermissionFlagsBits.Administrator)) {
    const messageId = message.content.split(" ")[1];

    if (!messageId) {
      return message.reply("❌ Usage: `!delete-rr-panel <message_id>`");
    }
    ...
    try {
      ...
      reactionRolePanels.delete(messageId);
      message.reply("✅ Reaction role panel deleted!");
    } catch (error) {
      console.error("Error deleting panel:", error);
      message.reply("❌ Failed to delete the panel message, but removed from tracking.");
      reactionRolePanels.delete(messageId);
    }
  }

  // Reaction Role Help
  if (message.content === "!rrhelp") {
    const rrHelpEmbed = new EmbedBuilder()
      .setColor("#5865F2")
      .setTitle("🎭 Reaction Role System")
      .setDescription("Manage self-assignable roles with button-based panels")
      .addFields(
        {
          name: "!create-rr-panel [title]",
          value: "Creates a new reaction role panel\n**Example:** `!create-rr-panel Game Roles`",
          inline: false
        },
        {
          name: "!add-rr <message_id> @role <emoji> [description]",
          value: "Adds a role to a panel\n**Example:** `!add-rr 123456 @Gamer 🎮 For gaming enthusiasts`",
          inline: false
        },
        {
          name: "!remove-rr <message_id> @role",
          value: "Removes a role from a panel\n**Example:** `!remove-rr 123456 @Gamer`",
          inline: false
        },
        {
          name: "!list-rr-panels",
          value: "Lists all reaction role panels",
          inline: false
        },
        {
          name: "!delete-rr-panel <message_id>",
          value: "Deletes a reaction role panel\n**Example:** `!delete-rr-panel 123456`",
          inline: false
        },
        {
          name: "📝 Notes",
          value: "• Max 25 roles per panel\n• Requires Manage Roles permission\n• Bot must have permission to manage the roles\n• Users click buttons to toggle roles on/off",
          inline: false
        }
      )
    ...
    message.reply({ embeds: [rrHelpEmbed] });
  }
    ...
    const selectMenu = new StringSelectMenuBuilder()
      ...
      .addOptions(
        ...
        new StringSelectMenuOptionBuilder()
          ...
          .setDescription("Self-assignable role system")
          .setValue("help_reactionroles")
          .setEmoji("🎭"),
        new StringSelectMenuOptionBuilder()
  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId === "help_menu") {
      ...
      switch (selectedValue) {
        ...
        case "help_reactionroles":
          helpEmbed = new EmbedBuilder()
            .setColor("#5865F2")
            .setTitle("🎭 Reaction Role System")
            .setDescription("Create self-assignable role panels with buttons")
            .addFields(
              {
                name: "!create-rr-panel [title]",
                value: "Creates a new reaction role panel with buttons\n**Example:** `!create-rr-panel Choose Your Roles`\n**Permissions:** Administrator",
                inline: false,
              },
              {
                name: "!add-rr <message_id> @role <emoji> [description]",
                value: "Adds a role to an existing panel\n**Example:** `!add-rr 1234567890 @Gamer 🎮 For gaming updates`\n**Permissions:** Manage Roles",
                inline: false,
              },
              {
                name: "!remove-rr <message_id> @role",
                value: "Removes a role from a panel\n**Example:** `!remove-rr 1234567890 @Gamer`\n**Permissions:** Manage Roles",
                inline: false,
              },
              {
                name: "!list-rr-panels",
                value: "Lists all active reaction role panels with their IDs\n**Permissions:** Administrator",
                inline: false,
              },
              {
                name: "!delete-rr-panel <message_id>",
                value: "Deletes a reaction role panel\n**Example:** `!delete-rr-panel 1234567890`\n**Permissions:** Administrator",
                inline: false,
              },
              {
                name: "!rrhelp",
                value: "Shows reaction role commands quick reference",
                inline: false,
              },
              {
                name: "📋 Features",
                value: "• Button-based role selection (click to toggle)\n• Support for up to 25 roles per panel\n• Custom emoji and descriptions\n• Automatic role management\n• Multiple panels supported",
                inline: false,
              },
            )
            .setFooter({ text: "Use the dropdown to view other categories" })
  ...
  if (interaction.customId.startsWith("rr_")) {
    try {
      ...
      if (role.position >= botMember.roles.highest.position) {
        return interaction.reply({
          content: "❌ I don't have permission to manage this role! It's higher than my highest role.",
          ephemeral: true
        });
      }
      ...
      if (member.roles.cache.has(roleId)) {
        ...
        const removeEmbed = new EmbedBuilder()
          .setColor("#ED4245")
          .setDescription(`✅ Removed role **${role.name}** from you!`)
          .setTimestamp();
        ...
        const addEmbed = new EmbedBuilder()
          .setColor("#57F287")
          .setDescription(`✅ Added role **${role.name}** to you!`)
          .setTimestamp();
      ...
      await interaction.reply({
        content: "❌ An error occurred while toggling the role. Make sure the bot has permission to manage roles!",
        ephemeral: true
      }).catch(() => {});
    }
    return;
